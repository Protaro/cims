<script lang="ts">
    ...
    let defaultMilestones = $state<Milestone[]>(
        data.milestones?.filter((m: any) => !m.isCustom) || []
    );
    let customMilestones = $state<Milestone[]>(
        data.milestones?.filter((m: any) => m.isCustom) ||[]
    );
    ...
    let contractStatus = $state(data.contractStatus || "On Hold");
    ...
    $effect(() => {
        data.milestones = [...defaultMilestones, ...customMilestones];
    ...
    function addCustomMilestone() {
        customMilestones =[
            ...customMilestones, 
            { text: `New Milestone`, done: false, isCustom: true }
        ];
    }

    function removeCustomMilestone(index: number) {
        customMilestones = customMilestones.filter((_, i) => i !== index);
    }
...
<div class="phase-container">
	...
	<div class="section-card">
        ...
        <!-- DEFAULT PREEXISTING MILESTONES -->
        {#if defaultMilestones.length > 0}
            <h3>Default Output Milestones</h3>
            <div class="checklist-items">
                {#each defaultMilestones as m}
        ...
        <!--  MODIFIABLE/APPENDABLE CUSTOM MILESTONES -->
        <h3>Additional Output Milestones</h3>
        <div class="checklist-items">
            {#each customMilestones as m, i}
                <div class="milestone-row">
                    <label class="custom-checkbox">
                        <input type="checkbox" bind:checked={m.done} />
                        <span class="checkmark"></span>
                    ...
                    <button class="delete-btn" onclick={() => removeCustomMilestone(i)}>
                        ×
                    </button>
                </div>
            {/each}
        ...
        <button class="pill-button" onclick={addCustomMilestone}>+ Add Milestone</button>
    </div>
	...
	<!-- CONTRACT TERMINATION (Only visible if status is Terminated) -->
    {#if contractStatus === "Terminated"}
