<script lang="ts">
    ...
    let defaultParties = $state<Party[]>(
        $contractStore.activation.parties.filter((p: any) => !p.isCustom)
    );
    let customParties = $state<Party[]>(
        $contractStore.activation.parties.filter((p: any) => p.isCustom)
    );
    ...
    $effect(() => {
        contractStore.update(s => ({
            ...
            activation: {
                ...s.activation,
                parties: [...defaultParties, ...customParties] as any
            }
        }));
        data.files = newFiles;
    ...
    function addCustomParty() {
        customParties =[...customParties, { name: "New Party", done: false, isCustom: true }];
    }

    function removeCustomParty(index: number) {
        customParties = customParties.filter((_, i) => i !== index);
    }
    ...
    function validateBeforeConfirm() {
        const allParties = [...defaultParties, ...customParties];
        ...
        const hasEmptyParty = allParties.some(party => party.name.trim() === "");
        if (hasEmptyParty) {
...
<div class="phase-container">
    <div class="checklist-section">
        ...
        <!-- DEFAULT PREEXISTING PARTIES -->
        {#if defaultParties.length > 0}
            <h3>Default Signing Parties</h3>
            {#each defaultParties as party}
        ...
        <!-- MODIFIABLE/APPENDABLE CUSTOM PARTIES -->
        <h3>Additional Signing Parties</h3>
        {#each customParties as party, i}
            <div class="checklist-row">
                <label class="custom-checkbox">
                    <input type="checkbox" bind:checked={party.done} />
                    <span class="checkmark"></span>
                ...
                <button class="delete-btn" onclick={() => removeCustomParty(i)}>
                    ×
                </button>
            </div>
        {/each}

        <button class="add-button-field" onclick={addCustomParty}>
            + Add Party
        </button>
    </div>
